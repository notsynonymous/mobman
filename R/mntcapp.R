...
mntcapp <- function(){
    ...
    vehicleType <- list("All Types", "Electric", "Regulated", "Commercial Van", "Step Van", "Non CV, SV, or EV")
    ...
    sidebarLayout(
        sidebarPanel(
        textInput("vin_numnbers", "Enter the VIN numbers seperated by a comma: ", "VIN NUMBERS"),
    ...
    server <- function(input, output, session) {
        ...
        output$concatForm <- renderUI({
            str0 <- paste0(input$vin_numbers, " have ")
