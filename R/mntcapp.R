...
#'
#' @importFrom shiny fluidPage tags HTML div sidebarLayout sidebarPanel selectInput mainPanel htmlOutput renderUI shinyApp
#'
#' @returns None
#' 
#' @examples
...
mntcapp <- function(){
  ...
  ui <- fluidPage(
    ...
    sidebarLayout(
      sidebarPanel(
        selectInput("category.main", "Main Damage Categories", choices = master_main.init),           #1
        selectInput("category.sub", "Damage Subcategories", choices = master_sublist.init),           #2
        selectInput("severity", "Level of Severity", choices = severity),                             #3
        selectInput("grounded", "Grounding Status", choices = grounded),                              #4
        selectInput("vehicleType", "Type of Vehicle Being Serviced", choices = vehicleType),          #5
        selectInput("modelNumber", "Model Number (select most applicable)", choices = modelNumber)    #6
      ),
      mainPanel(
  ...
  server <- function(input, output, session) {
    ...
    output$concatForm <- renderUI({
        str1 <- paste0("*PLACEHOLDER_FOR_VIN_NUMBERS* have ", tolower(input$category.sub))
